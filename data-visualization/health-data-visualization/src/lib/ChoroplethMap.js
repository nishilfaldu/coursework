...
/**
 * ChoroplethMap — a county-level US map shaded by a selected health attribute.
 * Rendered twice (num=1, num=2) for side-by-side comparison; brushing a region
 * pushes the selected counties into the shared CentralDataStore so the other
 * charts highlight the same counties.
 */
export class ChoroplethMap {
    ...
    updateVis(attribute) {
        ...
        vis.colorScale = d3
            .scaleLinear()
            .domain(d3.extent(filteredData, (d) => d.properties[attribute]))
        ...
        vis.legendRect.attr("fill", `url(#legend-gradient-${vis.num})`);
        //   .style(
        //     "display",
        //     vis.attributeName === "urban_rural_status" ? "none" : "block"
        //   );
    ...
    renderVis(attribute) {
        ...
        vis.counties = vis.countiesGroup
            .selectAll("path")
            .data(
                topojson.feature(vis.data, vis.data.objects.counties).features,
            )
            .join("path")
            .attr("d", vis.path)
            .style("fill", (d) => {
