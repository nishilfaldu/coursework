...
/**
 * Histogram — distribution of counties across the values of the selected
 * X-attribute (rendered as the "BarChart"). Brushing a value range filters the
 * shared CentralDataStore so the map and scatter plot react.
 */
export class Histogram {
    ...
    renderVis(attribute) {
        ...
        vis.chart
            ...
            .attr(
                "height",
                (d) => vis.config.containerHeight - vis.yScale(d.length),
            )
    ...
    brushed(_res, _vis) {
        ...
        if (!extent) {
            ...
            const range = [
                _vis.xScale.invert(extent[0]),
                _vis.xScale.invert(extent[1]),
            ];
