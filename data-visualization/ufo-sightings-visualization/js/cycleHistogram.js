/**
 * Annual-cycle histogram: counts of sightings grouped by month, day of
 *  the week, or season.
 */
...
class CycleHistogram {
  constructor(_config, _data, _dataStore) {
    ...
    d3.selectAll("#annual-cycle-dropdown").on("change", (event) =>
      this.updateVis(),
    );
    this.initVis();
  }
    ...
    vis.bars = vis.chart
      ...
      .on("mouseover", function (event, d) {
        vis.tooltip
          ...
          .html("<strong>Number of UFO Sightings: </strong>" + d)
  ...
  GetSeason(date) {
    ...
    if (springMonths.includes(month)) return 0;
    else if (summerMonths.includes(month)) return 1;
    else if (fallMonths.includes(month)) return 2;
    else if (winterMonths.includes(month)) return 3;
    else return -1;
  }
