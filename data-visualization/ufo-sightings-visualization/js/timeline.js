/**
 * Timeline area chart of sightings over time, with a draggable brush
 *  used to filter every other view to a date range.
 */
class Timeline {
  /**
   ...
   * @param {Array}
   */
  constructor(_config, _data, _actualData, _filterDataMain, _dataStore) {
    this.config = {
    ...
    vis.tooltipTrackingArea
      ...
      .on("mousemove", function (event) {
        ...
        vis.tooltip
          ...
          .text(
            Math.round(d.close) +
              " UFO sighting(s) during " +
              vis.monthNames[d.date.getMonth()] +
              " " +
              d.date.getFullYear(),
          );
      });
