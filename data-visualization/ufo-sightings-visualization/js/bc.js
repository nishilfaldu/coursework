/**
 * Bar chart of how frequently each encounter-length bucket occurs.
 */
class BC {
  constructor(opts) {
    const {
  ...
  init() {
    ...
    this.yScale = d3.scaleLog().range([this.height, 0]).clamp(true);
  ...
  updateVis() {
    ...
    const bars = this.svg.selectAll(".bar").data(this.binnedData);
    ...
    bars
      .on("mouseover", (event, d) => {
        this.tooltip
          .html(
            `<strong>Encounter Length (in thousands): </strong>${d.label}<br><strong>Frequency (in thousands): </strong>${d.value}`,
          )
