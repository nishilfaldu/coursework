/**
 * StackedBarchart — dialogue counts per character, stacked by season, for the
 * top characters. Driven by characters_by_season.json.
 */
const colorScaleForShapes_ = [
  "#FF0000",
  "#FF3300",
  "#FF6600",
  "#FF9900",
...
class StackedBarchart {
  ...
  initVis() {
    ...
    vis.groups = d3.map(vis.transformedData, function (d) {
      return d.group;
    });
    ...
    // Add Y axis
    var y = d3.scaleLinear().domain([0, 25000]).range([vis.height, 0]);
    vis.chart.append("g").call(d3.axisLeft(y));

    // color palette = one color per subgroup
    var color = d3.scaleOrdinal().domain(vis.subgroups).range(d3.schemeSet2);

    //stack the data? --> stack per subgroup
    var stackedData = d3.stack().keys(vis.subgroups)(vis.transformedData);
