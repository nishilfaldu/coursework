/**
 * WordLengthHisto — distribution of dialogue line lengths for a chosen pair of
 * characters (speaker -> listener), driven by full_conversation_data.json.
 */
class WordLengthHisto {
  constructor(_config, _data) {
    ...
    d3.selectAll(".histo-character-dropdown").on("change", (event) =>
      this.updateVis(),
    );
  }
  ...
  updateVis() {
    ...
    for (let i = 0; i < vis.data.length; i++) {
      if (
        vis.data[i].speaker == vis.characterA &&
        (vis.data[i].prevSpeaker == vis.characterB ||
          vis.data[i].nextSpeaker == vis.characterB)
      ) {
        vis.characterAData = vis.characterAData.concat(
          vis.data[i].text
            .replace(/ *_[^)]*_ */g, "")
            .replace(/[.,\/#!$%\^&\*;:{}=\_`~()"?]/g, " ")
            .replaceAll("-", " ")
    ...
    vis.xScale.domain([
      1,
      d3.max(vis.characterAData, function (d) {
        return d.length;
      }),
    ]);
