/**
 * Build script (Node): reads the per-episode transcript JSON under
 * data/frasier_transcripts/season_* and emits the conversation graph
 * (nodes + links per season) to data/conversations_by_season.json, which the
 * ArcDiagram consumes. Run with `node lib/conversations_by_season.js`.
 */
const fs = require("fs");
